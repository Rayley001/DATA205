...
leaflet() %>%
  addTiles() %>%
  addHeatmap(data = city_sf,
             lat = ~latitude,
             lng = ~longitude,
             intensity = ~crime_count,
             radius = 20,           # slightly smaller radius
             blur = 10,             # lower blur for sharper edges
             max = max(city_sf$crime_count, na.rm = TRUE) * 0.5,  # reduce max to boost contrast
             group = "Crime Prevalence") %>%
  addCircleMarkers(data = city_sf,
                   lat = ~latitude,
                   lng = ~longitude,
                   radius = 5,
                   color = "blue",
                   stroke = TRUE,
                   fillOpacity = 0.7,
                   popup = ~paste("City:", city, "<br>Crimes:", crime_count),
                   group = "Cities") %>%
  addLayersControl(
    overlayGroups = c("Crime Prevalence", "Cities"),
...
library(leaflet.extras)
library(htmltools)
crime_sf_clean <- crime_sf |>
  filter(!is.na(longitude), !is.na(latitude),
         longitude != 0, latitude != 0)

leaflet() |>
  addTiles() |>
  addHeatmap(data = crime_sf_clean,
             lat = ~latitude, 
             lng = ~longitude,
             radius = 15, 
             blur = 10, 
